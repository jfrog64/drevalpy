"""Tests for the configurable gene list of the PharmaFormer model."""
...
def gene_list_data_dir(tmp_path) -> str:
    """
    Build a minimal data directory with everything PharmaFormer's cell line loader reads.
...
def _build(hyperparameters: dict | None = None) -> PharmaFormerModel:
    """
    Instantiate and build a PharmaFormer model.
...
def test_pharmaformer_gene_list_default() -> None:
    """Without the hyperparameter the model keeps the previously hard-coded gene list."""
    assert MODEL_FACTORY["PharmaFormer"] is PharmaFormerModel
    assert PharmaFormerModel.gene_list == "landmark_genes_reduced"
    assert _build().gene_list == "landmark_genes_reduced"


def test_pharmaformer_default_gene_list_is_used_when_loading(gene_list_data_dir) -> None:
    """
...
def test_pharmaformer_gene_list_hyperparameter(gene_list_data_dir) -> None:
    """
...
def test_pharmaformer_gene_list_none_loads_all_genes(gene_list_data_dir) -> None:
    """
...
def test_pharmaformer_gene_list_does_not_leak_between_instances(gene_list_data_dir) -> None:
    """
...
def test_pharmaformer_gene_list_survives_the_save_load_round_trip(gene_list_data_dir, tmp_path) -> None:
    """
    load() restores the hyperparameters without calling build_model, so it must set the gene list itself.

    Otherwise the restored model would predict on the class default gene space instead of the one
    it was trained on.

    :param gene_list_data_dir: path to the temporary data directory
    :param tmp_path: pytest temporary path fixture
    """
    trained = _build({"gene_list": "gene_expression_intersection"})

    # This is what save() writes, without the parts that need a built network.
    directory = tmp_path / "model"
    directory.mkdir()
    with open(os.path.join(directory, "hyperparameters.json"), "w") as handle:
        json.dump(trained.hyperparameters, handle)

    restored = PharmaFormerModel.load(str(directory))
